...
class ExcelService {
  ...
  static Future<String> exportPanelReport(String panelSerial) async {
    ...
    // get panel metadata
    List<Map<String, dynamic>> panelResult =
    await db.query(
      "panels",
      where: "panel_serial = ?",
    ...
    panelResult.isNotEmpty ? panelResult.first : null;

    // Title
    sheet.appendRow([
      TextCellValue("TRACEABILITY REPORT")
    ]);
    ...
    // Panel Serial
    sheet.appendRow([
      TextCellValue("Panel ID : $panelSerial")
    ]);
    ...
    sheet.appendRow([
      ...
      TextCellValue(panelData?['start_date'] ?? "")
    ]);

    sheet.appendRow([
      TextCellValue("End Date"),
      TextCellValue(panelData?['end_date'] ?? "")
    ]);

    sheet.appendRow([
      TextCellValue("Project Name"),
      TextCellValue(panelData?['project_name'] ?? "")
    ]);

    sheet.appendRow([
      TextCellValue("Panel Sr No"),
      TextCellValue(panelSerial)
    ]);

    sheet.appendRow([
      TextCellValue("Prepared By"),
      TextCellValue(panelData?['prepared_by'] ?? "")
    ]);

    sheet.appendRow([
      TextCellValue("Verified By"),
      TextCellValue(panelData?['verified_by'] ?? "")
    ]);

    sheet.appendRow([
      TextCellValue("Approved By"),
      TextCellValue(panelData?['approved_by'] ?? "")
    ]);

    sheet.appendRow([
      TextCellValue("Remarks"),
      TextCellValue(panelData?['remarks'] ?? "")
    ]);
    ...
    String newPath =
        dir!.path.split("Android")[0] + "Download";

    final file =
    File("$newPath/traceability_$panelSerial.xlsx");
